...
import { getTopRatedMovies, getGenres } from '@/services/tmdb';
import { ResponseData } from '@/interfaces';

export async function getStaticProps() {
  const queryClient = new QueryClient();

  await queryClient.prefetchQuery(['top-rated-movies'], () => getTopRatedMovies());
...
export default function TopRated({ genres }: { genres: { id: number; name: string }[] }) {
  const [selectedGenre, setSelectedGenre] = useState<number | null>(null);

  const { data, isFetching, isFetchingNextPage, fetchNextPage } = useInfiniteQuery<ResponseData>({
    queryKey: ['top-rated-movies', selectedGenre],
    queryFn: ({ pageParam = 1 }) =>
      getTopRatedMovies(pageParam).then((movies) =>
        selectedGenre
          ? {
              ...movies,
  ...
  return (
    <>
      ...
      <div className="p-6 h-[1080px] bg-gray-900 text-white">
        <h2 className="text-3xl text-center font-semibold m-4 md:m-8 mb-8">Top Rated Movies</h2>
